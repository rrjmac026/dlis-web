...
export default function MinutesForm({ minutes, basePath, sessionTypes }: Props) {
    const isEditing = Boolean(minutes);
    const value = (key: keyof MinutesRecord): string | number => {
        const current = minutes?.[key];
                    ...
                    <section className="grid gap-6 rounded-lg border p-6 md:grid-cols-2">
                        <div className="grid gap-2">
                            ...
                            <select
                                id="session_type"
                                name="session_type"
                                defaultValue={value('session_type')}
                                required
