...
class AdminResolutionController extends Controller
{
    ...
    public function downloadDocument(Resolution $resolution)
    {
        $url = $this->documents->resolveUrl($resolution->document_path);
        abort_if(! $url, 404);

        $filename = $resolution->resolution_number . '.' . pathinfo(parse_url($url, PHP_URL_PATH) ?? '', PATHINFO_EXTENSION);
